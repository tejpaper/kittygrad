...
def _sigmoid(_ctx: Context, tensor: Tensor) -> Tensor:
    return tsr.tensor(
        data=1 / (1 + strict.exp(-tensor._data)),
        dtype=tensor.dtype,
...
def _softmax(ctx: Context, tensor: Tensor, dim: int | Size | None) -> Tensor:  # TODO: test
    ...
    exp = strict.exp(tensor._data - np.max(tensor._data))
    probs = strict.divide(exp, np.sum(exp, axis=dim, keepdims=True))
